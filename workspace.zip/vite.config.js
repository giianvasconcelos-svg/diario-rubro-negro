  ...
  return {
    ...
    transform(code, id) {
      ...
      const replaceRequired = (source, replacement, label) => {
        if (!code.includes(source)) throw new Error(`[drn-editorial-app-patch] Não foi possível aplicar: ${label}`);
  ...
  return {
    ...
    transform(code, id) {
      if (/[\\/]src[\\/]components[\\/]AutoNewsFetcher\.tsx$/.test(id)) {
        ...
        return { code: code.replace(source, `tags: Array.from(new Set([...reform.tags, '${marker}'])),`), map: null };
      }
      if (/[\\/]src[\\/]components[\\/]NewsImporter\.tsx$/.test(id)) {
        ...
        return { code: code.replace(source, `tags: Array.from(new Set([...editedTags, '${marker}'])),`), map: null };
      }
...
function adsenseHomeQualityPatch() {
  return {
    name: 'drn-adsense-home-quality',
    enforce: 'pre',
    transform(code, id) {
      if (!/[\\/]src[\\/]App\.tsx$/.test(id)) return null;
      const replaceRequired = (source, replacement, label) => {
        if (!code.includes(source)) throw new Error(`[drn-adsense-home-quality] Não foi possível aplicar: ${label}`);
        code = code.replace(source, replacement);
      };

      replaceRequired('🔴⚫ O maior portal da Nação Rubro-Negra', '🔴⚫ Portal independente sobre o Flamengo', 'descrição institucional');
      replaceRequired("{ name: 'Fórum', page: 'forum' as const },", "{ name: 'Sobre', page: 'home' as const },", 'remoção do fórum demonstrativo');
      replaceRequired(
        "onClick={() => { if (item.name === 'Contato') { window.location.href = '/contato'; return; } setActivePage(item.page); setMenuOpen(false); }}",
        "onClick={() => { if (item.name === 'Contato') { window.location.href = '/contato'; return; } if (item.name === 'Sobre') { window.location.href = '/sobre'; return; } setActivePage(item.page); setMenuOpen(false); }}",
        'navegação Sobre'
      );
      replaceRequired('className="bg-black text-white py-2 overflow-hidden"', 'className="hidden bg-black text-white py-2 overflow-hidden"', 'ticker demonstrativo');
      replaceRequired('className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8"', 'className="hidden grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8"', 'hero demonstrativo');
      replaceRequired('{/* Content Grid */}\n            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">', '{/* Content Grid */}\n            <div className="hidden grid-cols-1 lg:grid-cols-3 gap-6">', 'conteúdo estático demonstrativo');
      replaceRequired('<div className="relative">\n                <input', '<div className="hidden">\n                <input', 'busca ainda não funcional');
      replaceRequired('publishedNews.slice(0, 4)', 'publishedNews.slice(0, 12)', 'mais matérias reais na home');

      const socials = [
        ['instagram', 'Instagram'], ['twitter', 'X/Twitter'], ['youtube', 'YouTube'], ['tiktok', 'TikTok']
      ];
      for (const [icon, label] of socials) {
        const source = `<a href="#" className="hover:text-red-400 transition-colors"><i className="fab fa-${icon}"></i></a>`;
        const replacement = `<span className="hidden" aria-label="${label}"><i className="fab fa-${icon}"></i></span>`;
        if (code.includes(source)) code = code.replace(source, replacement);
      }
      return { code, map: null };
    },
  };
...
export default defineConfig({
  plugins: [adsenseEditorialGuardPatch(), editorialAppPatch(), adsenseHomeQualityPatch(), react(), tailwindcss()],
  server: {
    host: "0.0.0.0",
    port: 3000,
    strictPort: true,
